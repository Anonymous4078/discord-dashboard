...
router.get('/server/:guildID/profile', checkAuth, async (req, res) => {
  const userObj = req.client.guilds.cache
  ...
  if (userObj.presence === null) {
    ...
    statusGameData = userObj.presence.activities[0]
      ? userObj.presence.activities[0].name
      : 'Not Playing/Streaming';
...
router.get('/server/:guildID', checkAuth, async (req, res) => {
  ...
  const serverData =
    (await db.findServer(req.params.guildID)) ||
    (await db.createServer(req.params.guildID));
  ...
  if (
    ...
    return res.redirect(
      `https://discord.com/oauth2/authorize?client_id=${req.client.user.id}&scope=bot%20applications.commands&permissions=1098974625783&guild_id=${req.params.guildID}`,
    );
  } else if (!server) {
